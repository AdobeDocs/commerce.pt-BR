...
>[!IMPORTANT]
>
>As configurações do não são importadas automaticamente. Antes de iniciar o processo de migração de dados, anote as definições atuais de configuração do Catálogo no [!DNL Commerce Admin]. Em seguida, implemente a mesma configuração no ambiente de destino [!DNL Adobe Commerce as a Cloud Service].
