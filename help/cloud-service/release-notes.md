...
Os seguintes itens estarão disponíveis nos ambientes de Produção em 22 de setembro de 2026.

>[!BEGINSHADEBOX]

### Anexar arquivos e imagens para retornar solicitações

Agora os clientes podem carregar arquivos e imagens ao enviar uma solicitação de retorno por meio da mutação do GraphQL da vitrine `requestReturn`. <!-- CCSAAS-5410 -->

### Controlar aparência da origem do inventário

Cada origem de inventário agora inclui um botão de alternância [!UICONTROL **Visível na Loja**] na página de edição de origem em [!DNL Commerce Admin] ([!UICONTROL **Lojas**] > [!UICONTROL **Inventário**] > [!UICONTROL **Fontes**]). A consulta do GraphQL [`sourceAvailability`](https://developer.adobe.com/commerce/webapi/graphql/schema/products/queries/source-availability) retorna informações de estoque somente para fontes que você sinaliza como visíveis. As fontes estão ocultas por padrão. <!-- ACCS-1645 -->

### Guia de remessas de várias origens

Quando um pedido contém itens indicados a diferentes fontes de inventário, a página [!DNL Commerce Admin] [!UICONTROL **Seleção de Source**] agora orienta automaticamente você pela seleção da origem apropriada para cada item. <!-- ACCS-1832 -->

### Detalhes de disponibilidade da fonte de consulta para a loja

A consulta do GraphQL [`sourceAvailability`](https://developer.adobe.com/commerce/webapi/graphql/schema/products/queries/source-availability) agora retorna informações adicionais para vitrines:

* Cada origem inclui seu sinalizador `name` e `is_pickup_location_active`, portanto, as vitrines podem exibir nomes de origem e identificar locais de coleta ativos na loja. <!-- ACCS-1614 -->

* O tipo `SourceAvailability` agora inclui os campos `is_saleable` e `backorder_message`, portanto as vitrines podem [!UICONTROL Add to Cart] na verdadeira viabilidade de venda em vez de estoque físico. <!-- ACCS-1858 -->

### Recuperar permissões do cliente

Um novo ponto de extremidade REST [!DNL Commerce] (`GET /V1/customers/:customerId/companyRoles`) retorna todas as funções e permissões de empresa atribuídas a um cliente. O tipo `CompanyBasicInfo` do GraphQL agora também inclui dados de `role_id`, `role_name` e permissão, para que as integrações possam recuperar as atribuições de função na empresa de um usuário em todas as contas associadas em uma única solicitação. <!-- ACCS-1617 -->

### Assinar um webhook para salvar fatura

O webhook `observer.sales_order_invoice_save_after` permite executar a lógica após salvar uma fatura. <!-- CEXT-6706 -->

### Exibir informações de origem nomeada

Quando um pedido contém itens com uma origem de estoque designada, o [!DNL Commerce Admin] agora rotula esses itens na página de exibição do pedido e na tela de seleção da origem da remessa, para que os comerciantes possam atender aos pedidos a partir da origem correta. <!-- ACCS-941 -->
...
* Carrinho otimizado e carregamento de preço no nível de checkout para evitar problemas com um grande número de catálogos compartilhados. <!-- ACCS-1150 -->

* Correção de um problema em que as alterações de atributo de arquivo não eram respeitadas. <!-- CCSAAS-5395 -->

* Correção de um problema em que os dados de substituição de produto podiam estar inconsistentes. <!-- ACCS-1844 -->

* Correção de um problema em que as solicitações simultâneas da API REST podiam causar um erro 401 de forma intermitente. <!-- CCSAAS-5417 -->

* Correção de um problema em que a data de início ou de término de uma regra de preço do carrinho podia funcionar inesperadamente quando não havia uma hora explícita definida. <!-- ACCS-1856 -->

* Correção de um problema em que uma regra de preço do carrinho aceitava datas de início ou término inexistentes. <!-- ACCS-1902 -->

* Correção de um problema que impedia salvar a configuração do [!UICONTROL Catalog]. <!-- CCSAAS-5436 -->

* Correção de um problema que poderia ocorrer ao fazer solicitações simultâneas para a API de importação (`POST /V1/import/json`). <!-- ACCS-1053 -->

* Correção de um problema em que a quantidade vendável poderia relatar em excesso a disponibilidade de produtos com uma origem de inventário nomeada. <!-- ACCS-1103 -->

* Correção de um problema em que a API de Comentário do Pedido (`POST /V1/orders/{id}/comments`) não respeitava a diferenciação entre maiúsculas e minúsculas. <!-- ACCS-1752 -->

{{accs-release}}

>[!ENDSHADEBOX]

## Setembro de 2026 - versão #1
...
### Adobe Commerce as a Cloud Service atualizado para 2.4.9
...
* Configurações B2B e da empresa

Esses pontos de extremidade permitem que os administradores sincronizem a configuração entre ambientes programaticamente, em vez de reconfigurar manualmente o [!DNL Commerce Admin]. A execução de `GET /V1/system/config` no ambiente de sandbox e a execução de `PUT /V1/system/config` com a configuração de sandbox recuperada anteriormente permite sincronizar as alterações de configuração de Sandbox para Produção. <!-- ACCS-607, CCSAAS-5346 -->
...
Os novos pontos de extremidade de modelo de `PUT` e `DELETE` [email personalizado](https://developer.adobe.com/commerce/webapi/rest/saas-integrations/custom-email/) permitem que as integrações atualizem e excluam modelos de email personalizados. <!-- CCSAAS-5091 -->

### Exibir grupos de opções e identificadores de produtos por meio da REST API

A chamada REST `GET /V1/products/:sku/options` agora retorna os campos `group` e `option_uids` para cada opção, correspondendo aos identificadores já disponíveis no GraphQL. <!-- ACCS-1370 -->

### Novos eventos de catálogo compartilhado

Os seguintes eventos de catálogo compartilhados estão disponíveis para assinatura usando [!DNL Adobe I/O Events]:<!-- ACCS-1532 -->

* Atribuição de categoria (`observer.shared_catalog_assign_categories`)
* Cancelamento de atribuição de categoria (`observer.shared_catalog_unassign_categories`)
* Atribuição de empresa (`plugin.magento.shared_catalog.api.company_management.assign_companies`)
* Cancelamento de atribuição da empresa (`plugin.magento.shared_catalog.api.company_management.unassign_companies`)
* Todos sem atribuição da empresa (`plugin.magento.shared_catalog.api.company_management.unassign_all_companies`)
* Salvar catálogo compartilhado (`plugin.magento.shared_catalog.api.shared_catalog_repository.save`)
* Exclusão de catálogo compartilhado (`plugin.magento.shared_catalog.api.shared_catalog_repository.delete`)

### Usar endereços da empresa em ordens de compra, cotações e devoluções

Os Catálogos de endereços da empresa agora se integram a workflows B2B adicionais. As empresas que usam um catálogo de endereços compartilhado veem endereços consistentes no escopo da empresa em:
...
Para obter informações detalhadas, incluindo mutações do GraphQL e pontos de extremidade REST, consulte o [changelog de Pacote B2B de Compatibilidade de Loja](https://experienceleague.adobe.com/en/tools/commerce-storefront/releases/changelog/#storefront-compatibility-b2b-package-v1-0-24-2026-07-20).

<!-- USF-3629, USF-4187, USF-4188, USF-4189, USF-4191, USF-4192, USF-4193, USF-4194, USF-4195 -->

### Ocultar imagens de exibições de armazenamento em [!DNL AEM Assets]

A integração do [!DNL AEM Assets] agora dá suporte a um parâmetro `hiddenStoreViews`, portanto, as imagens importadas podem ter o escopo oculto em exibições de loja específicas. Isso permite que você mostre diferentes imagens de produto para diferentes lojas regionais ou demográficas. <!-- ACAP-1308 -->
...
### Registrar edições de pedidos no histórico de pedidos
...
Quando um pedido é editado, o Commerce agora pode adicionar um comentário legível ao histórico do novo pedido, que resume o que foi alterado em relação ao pedido substituído. <!-- ACCS-1157 -->

### Melhorias e correções de erros

Os seguintes aprimoramentos, otimizações e correções de erros selecionados estão incluídos nesta versão:

* Correção de um problema que poderia ocorrer quando pedidos de convidados eram feitos usando o email de um cliente registrado. <!-- CCSAAS-5313 -->

* Correção de um problema em que execuções repetidas de exportação de dados podiam causar problemas de recursos. <!-- CCSAAS-5275 -->

* Correção de um problema de fallback com rótulos da galeria de mídia do GraphQL na integração [!DNL AEM Assets]. <!-- ACAP-1308 -->

* Correção de um problema com [!DNL PayPal] parâmetros SDK que poderia afetar a renderização do check-out. <!-- PAY-6961 -->

* Correção de um problema em que métodos de pagamento [!DNL Payment Services] sem suporte poderiam aparecer no check-out. <!-- PAY-6976 -->

* Correção de um problema em que as cargas de evento com campos com valor de matriz, como categoria de catálogo compartilhado e eventos de atribuição de empresa, podiam conter objetos vazios em vez dos dados esperados. <!-- CEXT-6554 -->

* Correção de um problema em que a configuração de vários atributos pesquisáveis de endereço do cliente ou do cliente podia causar erros de exibição. A grade do cliente agora o notifica quando o limite é atingido. <!-- CCSAAS-5303 -->

* Correção de um problema em que os URLs de imagem de categoria eram quebrados no feed de exportação de dados de categoria. <!-- ACCS-1571 -->

* Correção de um problema em que a atribuição ou o cancelamento simultâneo da atribuição de produtos em diferentes catálogos compartilhados podia falhar intermitentemente. <!-- CCSAAS-5287 -->

* A configuração **Usar nas Opções de Pesquisa** como Não, mantém o atributo visível como uma coluna de grade, mas não conta no limite de 16 colunas. <!-- CCSAAS-5370 -->

* Solução de um problema que poderia afetar a navegação do Administrador. <!-- CCSAAS-5232 -->

* Correção de um problema em que os carrinhos vazios podiam relatar totais diferentes de zero. <!-- ACCS-1730 -->
...
A página de detalhes do pedido [!DNL Commerce Admin] agora exibe a sequência completa de modificação de um pedido que inclui o pedido original e todos os pedidos filho criados por meio de edições subsequentes. Os comerciantes podem navegar entre ordens, alternar a visibilidade de ordens canceladas e acessar todas as faturas, remessas, avisos de crédito e comentários de ordem associados na exibição de sequência.<!-- ACCS-968 -->
...
Agora você pode usar a [Ferramenta de desenvolvimento de codificação de IA](https://developer.adobe.com/commerce/extensibility/developer-agent/coding-tools/){target="_blank"} para criar novos aplicativos [!DNL App Builder] e converter extensões PHP [!DNL Adobe Commerce] existentes em aplicativos [!DNL App Builder]. Os seguintes tutoriais estão disponíveis para demonstrar como usar as ferramentas:
...
* [!DNL Commerce Storefront on Edge Delivery Services] agora inclui [componentes B2B](https://experienceleague.adobe.com/en/tools/commerce-storefront/dropins-b2b/). Os seguintes menus suspensos B2B agora estão disponíveis:

  * **[Gerenciamento da empresa](https://experienceleague.adobe.com/en/tools/commerce-storefront/dropins-b2b/company-management/)** - Habilita o gerenciamento de perfis da empresa e permissões com base em funções para vitrines da Adobe Commerce.
  * **[Alternador de empresa](https://experienceleague.adobe.com/en/tools/commerce-storefront/dropins-b2b/company-switcher/)** - Fornece um componente de interface do usuário para que os usuários alternem entre várias empresas às quais estão associados.
  * **[Ordens de compra](https://experienceleague.adobe.com/en/tools/commerce-storefront/dropins-b2b/purchase-order/)** - Gerencia fluxos de trabalho de ordem de compra, regras de aprovação e histórico de ordens de compra para transações B2B.
  * **[Gerenciamento de cotações](https://experienceleague.adobe.com/en/tools/commerce-storefront/dropins-b2b/quote-management/)** - Habilita cotações negociáveis para clientes B2B com fluxos de trabalho de solicitação de cotação, negociação e aprovação.
  * **[Listas de requisições](https://experienceleague.adobe.com/en/tools/commerce-storefront/dropins-b2b/requisition-list/)** - Fornece ferramentas para criar e gerenciar listas de requisições para compras repetidas e pedidos em massa.
...
* Solução de um problema em que os atributos personalizados de produto não eram atualizados por meio da REST API quando [!DNL AEM Assets integration] era habilitado e o produto tinha imagens. <!-- ACAP-1178 -->
