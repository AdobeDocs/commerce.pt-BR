...
Os itens a seguir serão lançados para ambientes de Produção em 1 de abril de 2026.

>[!BEGINSHADEBOX]

### Verificar o status de assinatura do alerta de preço e estoque por meio do GraphQL

Novas consultas ao GraphQL, `isSubscribedProductAlertStock` e `isSubscribedProductAlertPrice`, permitem que as vitrines determinem se um comprador já está inscrito no estoque ou alertas de preço de um produto. <!-- ACCS-334 -->
...
A consulta `recaptchaFormConfigs` pode retornar a configuração para vários tipos de formulário em uma única solicitação em vez de exigir uma consulta por formulário. <!-- ACCS-628 -->
