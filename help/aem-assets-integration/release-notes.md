...
![Correção de um problema](../assets/fix.svg) em que os atributos da função de ativo (imagem, imagem pequena, miniatura e imagem de amostra) podiam acumular valores vazios no nível do modo de exibição de repositório, fazendo com que as imagens parecessem ausentes nos modos de exibição de repositório não padrão. Agora, esses atributos são globais no escopo e as linhas vazias existentes na exibição de armazenamento são removidas para que as imagens herdem corretamente da exibição de armazenamento padrão. <!-- Issue ACCS-1401 -->
...
![Correção de um problema](../assets/fix.svg) em que o cabeçalho da seção **[!UICONTROL OBSERVABILITY]** não estava presente no menu Admin **[!UICONTROL System]** quando a integração com o AEM Assets estava habilitada, deixando o link **[!UICONTROL Subscription List]** exibido sem seu cabeçalho. <!-- Issue ACAP-1302 -->

![Correção de um problema](../assets/fix.svg): correção de um problema em que salvar a configuração de integração do AEM Assets exibia um aviso de PHP bruto em vez de uma mensagem clara em instâncias nas quais a API de abertura do Dynamic Media não está habilitada. Uma mensagem de erro clara e genérica é exibida. <!-- ACAP-1298 -->

## v1.4.0

_12 de junho de 2026_

[!BADGE Com Suporte]{type=Informative tooltip="Compatível"} no Adobe Commerce versão 2.4.5 e posteriores.

![Novo problema](../assets/new.svg)<!-- Issue ACAP-1101 --> Melhoria na estabilidade geral e no desempenho do processamento de eventos por meio de chamadas de atualização de ativos em lote.
...
![Novo problema](../assets/new.svg)<!-- Issue ACAP-1246 --> O administrador do Commerce inclui uma lista [!UICONTROL Sync Status] centrada em ativos para pesquisar, filtrar, classificar e revisar o AEM Assets sincronizado por atributos de ativos em vez de associações de produtos. Consulte [Exibir o status de sincronização do AEM Assets](get-started/sync-status.md){target=_blank}.
...
![Novo problema](../assets/new.svg)<!-- Issue ACAP-1268 --> Quando imagens ou ativos são importados do AEM para uma galeria de mídia do produto Commerce, o texto alternativo agora é transportado como parte da sincronização. O texto alternativo pode ser definido por exibição de loja no AEM Assets. Consulte [Texto alternativo localizado nos metadados do AEM Assets](metadata.md#localized-alt-text-in-aem-assets-metadata){target=_blank}.

## v1.3.5

_1º de abril de 2026_

[!BADGE Com Suporte]{type=Informative tooltip="Compatível"} no Adobe Commerce versão 2.4.5 e posteriores.

![Correção de um problema](../assets/fix.svg)<!-- Issue ACAP-1223 --> em que conjuntos duplicados de funções de imagens de produtos apareciam no Administrador para produtos configuráveis.

![Correção de um problema](../assets/fix.svg)<!-- Issue CCSAAS-4769 --> em `UpdateAssetImageRolesDataPatch` que causava uma violação de restrição de integridade durante atualizações ou patches de dados.
