...
![Novo problema](../assets/new.svg)<!-- Issue ACCS-169 --> Os campos **[!UICONTROL Program ID]** e **[!UICONTROL Environment ID]** agora são preenchidos automaticamente como menus suspensos com base na sessão IMS do usuário. Isso elimina a entrada manual e reduz os erros de configuração. Para usar este recurso, o usuário deve ter uma função na [camada de entrega do AEM](https://experienceleague.adobe.com/pt-br/docs/experience-manager-cloud-service/content/onboarding/journey/assign-profiles-aem#add-team-members){target=_blank} e uma [ID do Cliente IMS do Seletor de Ativos](https://experienceleague.adobe.com/pt-br/docs/commerce/aem-assets-integration/get-started/setup-synchronization#configure-the-connection){target=_blank} válida configurada.
