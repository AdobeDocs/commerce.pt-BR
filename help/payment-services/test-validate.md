...
1. Conclua um check-out bem-sucedido de sua loja, com [Campos de Cartão de Crédito](payments-options.md#credit-card-fields) ou com qualquer um dos [botões de pagamento do PayPal](payments-options.md#paypal-payment-buttons). Consulte [Testando credenciais](#testing-credentials) para obter mais informações sobre como usar cartões de crédito falsos para testes.
1. Capture (quando sua ação de pagamento for [definida como `Authorize and Capture`](production.md#set-payment-services-as-payment-method)), [restitua](refunds.md) ou [anule](voids.md) a ordem recém-concluída. Você também pode simplesmente [criar uma fatura](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/order-management/invoices#create-an-invoice){target="_blank"} para um pedido, se sua ação de pagamento estiver definida como `Authorize` em vez de `Authorize and Capture`.
...
#### Desenvolvimento local do Apple Pay

O Apple Pay requer configuração adicional para desenvolvimento local. O Apple Pay usa o registro de domínio para verificar se seu site está autorizado a aceitar pagamentos do Apple Pay. Isso significa que o Apple deve ser capaz de acessar seu domínio para validar um arquivo de verificação de domínio em `/.well-known/apple-developer-merchantid-domain-association`.

Para o desenvolvimento local, seu ambiente deve atender aos seguintes requisitos:

* **Acessível publicamente**, o Apple deve ser capaz de acessar seu domínio pela Internet.
* **Protocolo HTTPS**, o Apple Pay somente funciona em conexões seguras.

Usar um serviço de túnel como [ngrok](https://ngrok.com/) satisfaz ambos os requisitos. Após configurar o ngrok conforme descrito acima, [registre seu domínio de sandbox](https://developer.paypal.com/docs/checkout/apm/apple-pay/#link-registeryoursandboxdomains) com o PayPal usando a URL **ngrok**.
...
### Testar pagamento do Apple na produção
