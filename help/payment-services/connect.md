...
Conecte sua instância do Commerce usando uma chave de API e uma chave privada e especifique o espaço de dados na configuração usando o [Commerce Services Connector](../landing/saas.md). **Você configurou esta conexão apenas uma vez.**
...
Se sua chave de API for perdida, um novo par de chaves de API deverá ser [gerado](../landing/saas.md#genapikey) e [aplicado](../landing/saas.md#createsaasenv) à configuração do Commerce Services Connector no Administrador. Se as chaves incorretas estiverem configuradas ou não houver nenhuma na configuração, uma caixa de diálogo de erro de verificação de conta será exibida nos Serviços de pagamento, notificando-o de que a conta não foi verificada.

Consulte uma [lista de Serviços Commerce disponíveis que usam a API](../landing/saas.md#availableservices).

Para saber como gerar uma chave de API para ambientes de sandbox ou produção, consulte [Credenciais](../landing/saas.md#apikey).
...
A mesma chave de API pode ser usada em várias instâncias, mas cada instância deve ter seu próprio [Espaço de dados SaaS](../landing/saas.md#saasenv).
...
1. Para configurar os Serviços da Commerce, siga as etapas descritas em [Configuração do SaaS](../landing/saas.md#saasenv).
...
[!DNL Payment Services] usa o [Commerce Services Connector](../landing/saas.md) para se conectar aos Serviços Commerce e implantar como SaaS. Este [!DNL Commerce Services Connector] se comunica através do ponto de extremidade em:
