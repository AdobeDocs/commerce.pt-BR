...
Essas notas de versão descrevem as versões mais recentes do [!DNL Payment Services].

O Adobe fornece suporte para a versão mais recente lançada dos Serviços de Pagamentos. As notas de versão para versões mais antigas são fornecidas para referência.
...
![Novo problema](../assets/new.svg)<!-- Issue PAY-6374 --> Agora, o botão **Configurações** no painel [!DNL Payment Services] redireciona para a página Configuração de administrador para métodos de pagamento, fornecendo um fluxo de trabalho mais simplificado para o gerenciamento de configurações de pagamento.

_19 de janeiro de 2026_

![Novo problema](../assets/new.svg)<!-- Issue PAY-6325 --> Agora, os comerciantes podem exibir a ID de vendedor do PayPal como uma coluna no [relatório de transações](reporting.md#transactions-report-view) para ajudar a identificar as transações que um cliente específico fez.
...
![Novo problema](../assets/new.svg)<!-- Issue PAY-5658 --> Agora, os comerciantes podem filtrar as transações por Detalhes do Pagamento no [relatório de transações](reporting.md#transactions-report-view), para obter dados mais detalhados e precisos sobre métodos de pagamento.

_15 de julho de 2024_

![Novo problema](../assets/new.svg)<!-- Issue PAY-5571 --> Agora, os comerciantes podem filtrar transações por email de cliente do Commerce no [relatório de transações](reporting.md#transactions-report-view). Insira o email do cliente para filtrar as transações desse email específico.

_9 de julho de 2024_

![Novo problema](../assets/new.svg)<!-- Issue PAY-5488 --> Agora, os comerciantes podem exibir a ID de cliente da Commerce como uma coluna no [relatório de transações](reporting.md#transactions-report-view) para ajudar a identificar as transações colocadas por um cliente específico. Além disso, os comerciantes podem filtrar o relatório de transações por essa ID de cliente da Commerce para pedidos associados.
...
![Novo problema](../assets/fix.svg)<!-- Issue PAY-4888 --> Agora, os comerciantes podem filtrar as transações de cartão de crédito e débito pelos últimos quatro dígitos do número do cartão no [Relatório de transações](reporting.md#transactions-report-view).
...
![Novo](../assets/new.svg)<!-- Issue PAY-4288 --> Agora, os comerciantes podem [configurar _somente_ botões de pagamento do PayPal](payments-options.md#use-only-paypal-payment-buttons)—e _não_ usar a opção de pagamento com cartão de crédito do PayPal. Isso permite que os comerciantes forneçam várias opções de pagamento, incluindo os botões de pagamento Venmo e PayPal, e usem um fornecedor de cartão de crédito existente em vez da opção de pagamento com cartão de crédito PayPal.

![Novo](../assets/new.svg)<!-- Issue PAY-4050 --> Adicionou uma [visualização de dados](payouts.md#payouts-data-visualization-view), que aparece na Página Inicial do Serviço de Pagamento, para o relatório Status do pagamento da ordem.
...
![Problema corrigido](../assets/fix.svg)<!-- Issue PAY-4485--> As exibições de visualização de dados de relatório agora aparecem na Página Inicial [!DNL Payment Services] quando [!DNL Payment Services] está desabilitado.
...
![Novo](../assets/new.svg)<!-- Issue PAY-3705 --> O `increment_id` agora está disponível para uso na reconciliação de pagamentos em sistemas ERP externos. Ele é propagado para o `custom_id` _and_ `invoice_id`, visível no webhook do PayPal e nos detalhes da atividade do comerciante para um pagamento.
...
![Correção de um problema](../assets/fix.svg)<!-- PAY-6769 --> que corrigiu um problema intermitente em vitrines EDS que usavam botões inteligentes do PayPal e que fazia com que alguns métodos de pagamento ficassem indisponíveis na página de check-out.

## v2.16.0

_27 de julho de 2026_

![Novo problema](../assets/new.svg)<!-- Issue PAY-6510 --> Os comerciantes agora podem realizar o autoatendimento de integração de uma conta do PayPal diferente no escopo do site diretamente do [!DNL Commerce Admin]. [!DNL Payment Services] A página inicial agora inclui um botão **[!UICONTROL Connect a different PayPal account for a website]**, que redireciona para a página Configuração de administrador para métodos de pagamento. Consulte [Conectar uma conta diferente do PayPal para um site](connect-website-account.md) para obter mais informações.

## v2.15.0

_15 de junho de 2026_

[!BADGE Com suporte]{type=Informative tooltip="Compatível"} Adobe Commerce versões 2.4.4 e mais recentes

![Novo](../assets/new.svg)<!-- PAY-6564 --> Agora, o check-out expresso do [!DNL Google Pay] pode mostrar métodos de envio na folha de pagamento do Google usando retornos de chamada de envio do lado do cliente; portanto, os compradores selecionam o envio antes de confirmarem o pagamento. Os comerciantes podem mostrar ou ocultar uma etapa separada de revisão do Google Pay usando o **[!UICONTROL Skip Review]** na [configuração do Google Pay](configure-admin.md#google-pay). Quando a etapa de revisão for ignorada, as opções de envio permanecerão na planilha quando o endereço da wallet ou o método de envio forem alterados.

![Novo](../assets/new.svg)<!-- PAY-6565 --> Agora, os compradores podem aplicar um código promocional na folha de pagamento expressa [!DNL Google Pay], independentemente de a etapa de revisão do Google Pay aparecer ou não. Somente um código pode ser aplicado por folha de pagamento, mesmo em lojas que permitem vários cupons por pedido. Consulte [Opções de pagamento](payments-options.md#google-pay-button) para obter mais informações.

![Novo](../assets/new.svg)<!-- PAY-6567 --> Agora, o check-out expresso do [!DNL Apple Pay] está disponível no Chrome, Firefox e Microsoft Edge, além do Safari. No desktop, os compradores podem digitalizar um código QR com um iPhone executando o iOS 18 ou posterior para concluir o pagamento na folha de pagamento do Apple. Os posicionamentos expressos (por exemplo, na página de detalhes do produto) devem ser habilitados na [configuração de Pagamento do Apple](configure-admin.md#apple-pay).

![Novo](../assets/new.svg)<!-- PAY-6566 --> Agora, os compradores podem aplicar ou remover um único código de desconto na folha de pagamento expressa [!DNL Apple Pay] e atualizar os totais na folha. Crie e ative as regras de preço do carrinho no Admin como de costume.

![Novo](../assets/new.svg)<!-- PAY-6151 --> Agora, o [!DNL Apple Pay], nas páginas de detalhes e check-out do produto Luma, é renderizado por meio do Payments SDK, de forma que a experiência corresponda à integração do PayPal mais recente usada para check-out expresso.

![Novo](../assets/new.svg)<!-- PAY-6611 --> Agora, a página de detalhes do pedido [!DNL Adobe Commerce] exibe detalhes de pagamento adicionais para transações processadas por meio do [!DNL Payment Services], incluindo [!DNL PayPal] ID da Transação, ID de depuração do PayPal, email do pagador, Qualificação para Proteção do Comerciante e códigos de verificação AVS/CVV. Consulte [3DS](security.md#3ds) para obter mais informações.

![Problema corrigido](../assets/fix.svg)<!-- PAY-6513 --> Corrigido um problema no qual a mensagem de erro [!DNL Apple Pay] ainda podia aparecer na página de êxito do pedido após uma tentativa de pagamento com falha e um check-out bem-sucedido posterior.

![Problema corrigido](../assets/fix.svg)<!-- PAY-6585 --> Corrigido um problema no qual a folha de pagamento expressa [!DNL Apple Pay] exibia um erro **[!UICONTROL Please select a state/region]** para endereços do Reino Unido e da Europa, nos quais o estado/região não é um campo obrigatório.

## v2.14.0

_26 de fevereiro de 2026_

[!BADGE Com suporte]{type=Informative tooltip="Compatível"} Adobe Commerce versões 2.4.4 e mais recentes

![Novo](../assets/new.svg)<!-- PAY-6400 --> Adicionada uma configuração **[!UICONTROL Line Items Enabled]** para habilitar ou desabilitar o envio de itens de linha e detalhamento de valor para o PayPal. Esta configuração ajuda a resolver problemas de check-out quando extensões de terceiros adicionam taxas personalizadas que não têm suporte no [!DNL Payment Services]. Consulte [Itens de linha](line-items.md) para obter mais informações.

![Novo](../assets/new.svg)<!-- PAY-6457 --> Adição de [botões de pagamento expresso no início do check-out](payments-options.md#express-checkout-buttons) para incentivar check-outs mais rápidos.

![Novo](../assets/new.svg)<!-- PAY-6458 --> Suporte adicionado para [Métodos de Pagamento Locais (LPMs)](payments-options.md#local-payment-methods) para opções de pagamento locais e específicas da região. Os comerciantes podem habilitar ou desabilitar LPMs disponíveis diretamente no [Commerce](configure-admin.md#local-payment-methods).

![Novo](../assets/new.svg)<!-- PAY-6377 --> Adicionado uma configuração de sandbox do [país do comprador](sandbox.md#buyers-country) para simular a localização do comprador e controlar quais métodos de pagamento são renderizados.

![Correção do problema](../assets/fix.svg)<!-- PAY-6458 --> Compatibilidade aprimorada com os filtros de fraude do PayPal ao transmitir dados de endereço de email do comprador, aprimorando os recursos de proteção contra fraude.

![Correção de um problema](../assets/fix.svg)<!-- PAY-6413 --> que corrigia um problema em que o check-out do PayPal não podia ser concluído para produtos virtuais devido à falta de um endereço de envio.

![Problema corrigido](../assets/fix.svg)<!-- PAY-6450 --> Corrigido um problema no qual os erros de check-out do botão PayPal não eram tratados corretamente nas páginas de detalhes do produto, minicart, carrinho e parte superior do check-out.

![Correção de um problema](../assets/fix.svg)<!-- PAY-6453 --> Desabilitar o fluxo de retornos de chamada de envio do lado do servidor no check-out do GraphQL para evitar conflitos no check-out.

![Problema corrigido](../assets/fix.svg)<!-- PAY-6456 --> Atualizado a descrição da configuração de [Pagamento de Apple](configure-admin.md#apple-pay) para esclarecer que a Apple rejeita pagamentos de domínios não verificados. Os comerciantes devem verificar todos os domínios em que os botões de pagamento do Apple são usados. Restrições de país se aplicam.

## v2.13.3

_14 de janeiro de 2026_

![Correção de um problema](../assets/fix.svg)<!-- PAY-6399 -->: correção de um problema em que o cartão com cofre errado era usado durante um check-out de administrador. Agora, o cartão selecionado é usado corretamente ao fazer um pedido.

## v2.13.2

_5 de janeiro de 2026_

![Correção de um problema](../assets/fix.svg)<!-- PAY-6390 -->: melhorias e otimizações gerais por meio da implementação da minificação aprimorada do JS.

## v2.13.1

_18 de dezembro de 2025_

![Correção de um problema](../assets/fix.svg)<!-- PAY-6355 --> Correção de um problema em que os comerciantes não podiam selecionar um método de envio durante o check-out.

![Correção de um problema](../assets/fix.svg)<!-- PAY-6347 --> Melhoria na resiliência do check-out ao remover chamadas de API desnecessárias.

![Correção de um problema](../assets/fix.svg)<!-- PAY-6368 --> Implementação de um mecanismo de fallback para carregar o SDK de Pagamentos quando o domínio principal estiver indisponível, garantindo que os campos de cartão de crédito permanecessem acessíveis durante o check-out.

## v2.13.0

_10 de novembro de 2025_

[!BADGE Com suporte]{type=Informative tooltip="Compatível"} Adobe Commerce versões 2.4.4 e mais recentes

![Novo](../assets/new.svg)<!-- PAY-xxxx --> Agora, [!DNL Payment Services] dá suporte ao modal **Check-out Único (OTC)** do PayPal, apresentando informações de contato e entrega incorporadas.

![Nova](../assets/new.svg)<!-- PAY-xxxx --> experiência móvel aprimorada por meio da **autenticação de alternância de aplicativo do PayPal**, juntamente com integração de API aprimorada para jornadas de usuário mais suaves. Este recurso só está disponível para clientes baseados em [!DNL Payment Services] dos EUA.

![Novo](../assets/new.svg)<!-- PAY-xxxx --> Adição de **autenticação segura 3D (3DS)** para que o Fastlane atendesse aos requisitos de **Autenticação de Cliente Forte (SCA)**. Isso permite que os comerciantes do Reino Unido e da UE [!DNL Payment Services] processem transações com a **autenticação 3DS**, aprimorando a prevenção de fraudes e garantindo a conformidade com os regulamentos regionais.

![Novo](../assets/new.svg)<!-- PAY-xxxx --> Agora, [!DNL Payment Services] comerciantes podem escolher entre **temas claros e escuros** para o componente de check-out do Fastlane, permitindo que as páginas de check-out correspondam ao design do site. Se os estilos personalizados não atenderem aos padrões de acessibilidade, o sistema reverterá automaticamente para as configurações padrão.

![Correção do problema](../assets/fix.svg)<!-- PAY-xxxx --> Correção do problema do carregador durante o **check-out administrativo com o desafio 3DS**.

![Correção de um problema](../assets/fix.svg)<!-- PAY-xxxx --> Melhoria na **validação ao armazenar a configuração de pagamento** via API.
...
![Corrigido o problema](../assets/fix.svg)<!-- PAY-6275 --> Recusou as transações [!DNL Fastlane] no modo de captura para não criar mais pedidos no Commerce.
...
![Problema corrigido](../assets/fix.svg)<!-- PAY-6164 --> Agora, [!DNL Payment Services] usa a moeda base para os métodos de envio disponíveis no [SSSC (retorno de chamada de envio no lado do servidor) do PayPal](https://developer.paypal.com/docs/multiparty/checkout/standard/customize/shipping-module/).

![Problema corrigido](../assets/fix.svg)<!-- PAY-6267 --> O bloco **Enviar para** fica oculto na página de check-out quando **Seleção na Loja (ISPU)** é selecionado.

![Problema corrigido](../assets/fix.svg)<!-- PAY-6271 --> Agora, [!DNL Payment Services] exibe os detalhes do cartão de crédito salvo do PayPal PayFlow na seção **Conta do cliente** > **Métodos de pagamento armazenados**.
...
![Novo](../assets/new.svg)<!-- PAY-6022 --> O [Fastlane](https://experienceleague.adobe.com/en/docs/commerce/payment-services/payments-checkout/payments-options) oferece uma compra mais rápida durante check-outs de convidados.
...
![Novo](../assets/new.svg)<!-- PAY-6227 --> Ao fazer check-out com o PayPal, [!DNL Payment Services] ignora a janela pop-up de confirmação de pedido para um processo de compra mais rápido. Isso é habilitado pelo [retorno de chamada de envio do lado do servidor](payments-options.md#server-side-shipping-callbacks-for-paypal-payment-buttons), que calcula o envio e os totais diretamente na página de revisão do PayPal.

![Novo](../assets/new.svg)<!-- PAY-6234 --> Adicionou um novo recurso para a opção de pagamento [Pagar Mais Tarde](https://experienceleague.adobe.com/en/docs/commerce/payment-services/payments-checkout/payments-options). Agora, o configurador de mensagens BNPL oferece mais flexibilidade na exibição de mensagens BNPL de Pagamento Posterior nas páginas de finalização de compra do cliente.
...
![Novo](../assets/new.svg)<!-- PAY-5938 --> Agora, o [!DNL Payment Services] permite que os comerciantes gerenciem configurações de pagamento para maximizar a flexibilidade em seus negócios. Esta versão melhora a capacidade de anexar [várias contas do PayPal](configure-admin.md#use-multiple-paypal-accounts) para as regiões e marcas que um comerciante aceita. Nossa equipe de vendas pode fornecer um link de integração para configurar os escopos de visualização do site e da loja.
...
![Novo](../assets/fix.svg)<!-- PAY-5789 --> O [!DNL Payment Services] agora oferece suporte à [autenticação Segura 3D com o Google Pay](security.md#3ds), aprimorando a segurança para comerciantes e clientes durante transações de pagamento.

![Correção](../assets/fix.svg)<!-- PAY-5703 --> A [!DNL Payment Services] adiciona a capacidade de [clientes salvarem cartões diretamente em sua **Minha Conta**](vaulting.md#vaulting-without-purchase), melhorando a conveniência e simplificando check-outs futuros. `Vault without purchase functionality might not be 100% compatible with Adobe Commerce 2.4.4 due to a known issue with` [`GraphQL authorization mechanisms`](https://developer.adobe.com/commerce/webapi/graphql/usage/authorization-tokens/).
...
![Correção](../assets/fix.svg)<!-- PAY-5792 --> [!DNL Payment Services] agora exibe descrições e endereços de cobrança para [cartões com cofre na página de check-out](vaulting.md), dando aos clientes mais visibilidade sobre seus métodos de pagamento salvos.
...
![Novo](../assets/new.svg)<!-- PAY-5380 --> O [!DNL Payment Services] aprimora a [Configuração na experiência de Administrador](configure-admin.md#general-configuration) para comerciantes, para um processo de integração mais fácil e intuitivo. Este recurso permite que os comerciantes redefinam suas [!DNL Payment Services] IDs.
...
![Correção](../assets/fix.svg)<!-- PAY-5492 --> corrigiu um problema quando um comerciante adicionava termos e condições personalizados à página de check-out. Durante um [check-out expresso](payments-options.md#standard-vs-advanced-payments-experience), um comprador agora pode aceitar esses termos e condições para concluir o check-out sem problemas.
...
![Novo](../assets/new.svg)<!-- PAY-4877 --> Agora, o [!DNL Payment Services] oferece suporte aos [recursos de preços L2/L3](levels-card-payment-transactions.md#level-2-and-level-3). Este recurso só está disponível para [!DNL Payment Services] clientes com preço do IC++ habilitado. Se quiser usar os dados de processamento L2/L3 para [!DNL Payment Services], entre em contato com o gerente de conta do [!DNL Payment Services].
...
![Os novos](../assets/new.svg)<!-- PAY-4868 --> comerciantes podem [configurar o Google Pay com êxito durante toda a experiência de compra](payments-options.md), de modo semelhante a outros botões de pagamento em [!DNL Payment Services] por meio do Administrador.

![Novo](../assets/new.svg)<!-- PAY-4381 --> [Os Serviços de Pagamento oferecem suporte ao Google Pay por meio do GraphQL](https://developer.adobe.com/commerce/webapi/graphql/payment-services-extension/), permitindo que os comerciantes tenham uma experiência headless Commerce com o método de pagamento Google Pay.

![Novo](../assets/new.svg)<!-- PAY-4878 --> Agora, o recurso de check-out básico do [!DNL Payment Services] é fornecido para comerciantes do Adobe Commerce e do Magento Open Source.[!DNL Payment Services] O agora oferece suporte a comerciantes com empresas em qualquer uma das 200 regiões geográficas do mundo.[!DNL Payment Services] o check-out básico fornece opções de débito/crédito, PayPal, Venmo (quando disponível) e PayLater (quando disponível) em uma integração de autoatendimento.

![Correção](../assets/fix.svg)<!-- PAY-5291 --> O recebimento da confirmação de pagamento para algumas transações pode estar atrasado. Nesse caso, agora os comerciantes podem obter um status de pagamento atualizado para um pedido. [Os serviços de pagamento detectam o status pendente de uma transação de pagamento](order-payment-status.md#payment-status-updates) em um pedido, detectando transações pendentes, monitorando proativamente essas transações e atualizando quando o status pendente foi capturado.
...
- na [exibição de detalhes do pedido](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/order-management/orders/order-processing#view-an-order) no Administrador do Commerce.
...
![Novo](../assets/new.svg)<!-- PAY-4381 --> [Os Serviços de Pagamento agora oferecem suporte à integração com o GraphQL](https://developer.adobe.com/commerce/webapi/graphql/payment-services-extension/). Com suporte da GraphQL para botões de pagamento do PayPal, campos hospedados e Apple Pay, o [!DNL Payment Services] agora oferece suporte a uma configuração headless Commerce.
...
![Novo](../assets/new.svg)<!-- PAY-4638 --> Adição de uma [integração com Signifyd](fraud-protection.md), que fornece serviços automatizados de proteção contra fraude.

![Novo](../assets/new.svg)<!-- PAY-3981 --> [Promoveu o Apple Pay a uma opção de pagamento separada](payments-options.md#apple-pay-button), fora dos botões de pagamento do PayPal, para aumentar a visibilidade do comprador da opção de pagamento e permitir que os comerciantes controlem a disposição e o estilo do Apple Pay.
...
![Nova](../assets/new.svg)<!-- PAY-4002 --> Adicionada funcionalidade para permitir que os comerciantes [classifiquem a ordem de suas opções de pagamento](configure-admin.md#paypal-payment-buttons) para priorizar determinadas opções de pagamento. Essa funcionalidade incentiva uma taxa de conversa de check-out mais alta.

![Novos](../assets/new.svg)<!-- PAY-4035 --> Comerciantes agora podem monitorar com eficiência a integridade de suas lojas e identificar problemas de transação usando o novo [Relatório de transações](reporting.md#transactions-report-view) disponível na home page de Administrador[!DNL Payment Services]. O relatório também apresenta dados sobre taxas de autorização de transações e tendências de transações negativas.
...
![Novo](../assets/new.svg)<!-- Issue xxx --> Adicionado [disponibilidade nos seguintes países e moedas associadas](compatibility.md#standard-vs-advanced-payment-services-experience): Austrália, França, Reino Unido.

![Novo](../assets/new.svg)<!-- Issue PAY-4296 --> Adicionado [recursos expandidos para funções de Administrador](configure-admin.md#configure-roles) para garantir que os usuários Administradores possam criar e gerenciar pedidos para clientes e possam visualizar[!DNL Payment Services] no menu Vendas.

![Novo](../assets/new.svg)<!-- Issue PAY-4236 --> Adicionado [anulação automática para pedidos com erros durante o check-out](checkout.md#order-auto-voided-if-error).

![Novo](../assets/new.svg)<!-- Issue PAY-4183 --> criou a funcionalidade de [mostrar o botão de opção de pagamento de cartão de crédito/débito](payments-options.md#paypal-debit-or-credit-card-button) na página de check-out.
...
![Correção](../assets/fix.svg)<!-- Issue PAY-4226 --> Corrigido um problema que impedia que novos [!DNL Payment Services] comerciantes usassem o check-out no Administrador.[!DNL Payment Services] A estava usando anteriormente a ID de cliente da Commerce, que não existe para novos clientes.

![Correção](../assets/fix.svg)<!-- Issue PAY-4205 --> Corrigido um problema que fazia com que o estado do endereço de remessa especificado fosse substituído pelo estado nas configurações de imposto padrão durante o check-out usando a [opção PayPal](payments-options.md#paypal-payment-buttons). Agora, os clientes podem ter seus pedidos entregues em um estado diferente daquele configurado como padrão nas configurações de imposto do comerciante.

![Correção](../assets/fix.svg)<!-- Issue PAY-4202 --> Corrigido um problema que impedia que os clientes usassem o cofre de cartões para concluir uma compra ou excluir um método de pagamento com cofre para uma loja [usando a ação de pagamento `Authorize and Capture`](production.md#set-payment-services-as-payment-method). Anteriormente, um erro &quot;ID do Provider Vault não encontrada&quot; aparecia quando o cliente tentava usar ou modificar seus cartões de crédito com cofre.
...
![Novo](../assets/new.svg)<!-- Issue PAY-3880 --> Agora um comprador pode [guardar (salvar) suas informações de cartão de crédito durante o check-out](vaulting.md) para usar em uma compra posterior para a mesma loja ou outra na mesma conta de comerciante.

Os ![Novos](../assets/new.svg)<!-- Issue PAY-3950 --> Comerciantes agora podem habilitar o [recurso Commerce de Compra Instantânea](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/point-of-purchase/checkout-instant-purchase) para suas lojas para que os compradores possam (usar [informações de cartão de crédito com cofre](vaulting.md)) agilizar o check-out.
...
![Novo](../assets/new.svg)<!-- Issue PAY-784 -->[!DNL Payment Services] agora inclui a capacidade de configurar uma conta comercial para [usar várias contas comerciais do PayPal](configure-admin.md#use-multiple-paypal-accounts). Isso permite que o comerciante opere suas lojas em vários países usando moedas diferentes ou use o Adobe Commerce para uma parte da sua empresa.

![Os novos](../assets/new.svg)<!-- Issue PAY-3231 --> comerciantes podem [adicionar um [!UICONTROL Soft Descriptor]](configure-admin.md) à configuração de sites ou exibições de loja individuais que são mostradas nos extratos bancários de transações do cliente para definir marcas, lojas ou linhas de produtos.

![Novo](../assets/new.svg)<!-- Issue PAY-3707 --> [Habilite ou desabilite campos de cartão de crédito e botões de pagamento do PayPal](configure-admin.md#paypal-payment-buttons) para check-out nas configurações[!DNL Payment Services].
...
![Problema conhecido](../assets/bug.svg)<!-- Issue PAY-0 --> [!DNL Payment Services] é incompatível com `commerce-data-export` v101.20 e superior, o que o torna incompatível com a [[!DNL Channel manager] extensão](https://experienceleague.adobe.com/en/docs/discontinued/using/commerce-channels) (descontinuado).
...
![Novo](../assets/new.svg)<!-- Issue PAY-2682 --> A extensão [!DNL Payment Services] para [!DNL Adobe Commerce] e [!DNL Magento Open Source] agora está disponível para comerciantes canadenses. Os comerciantes podem exibir a configuração de pagamentos em [francês](compatibility.md##standard-vs-advanced-payment-services-experience) ou [inglês](compatibility.md#standard-vs-advanced-payment-services-experience).

![Novo](../assets/new.svg)<!-- Issue PAY-2681 --> O [!DNL Payment Services] oferece suporte a [Dólares canadenses (CAD)](compatibility.md#standard-vs-advanced-payment-services-experience) para cartões de crédito e transações no PayPal.
...
![Problema corrigido](../assets/fix.svg)<!-- Issue PAY-2277 --> Agora é possível [habilitar ou desabilitar botões de pagamento individuais do PayPal](configure-admin.md#paypal-payment-buttons).
...
![Problema conhecido](../assets/bug.svg)<!-- Issue PAY-2842 --> Transações de cartão de crédito de teste [podem falhar com PayPal](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26825) ao processar pagamentos em um ambiente de sandbox.
...
![Problema conhecido](../assets/bug.svg)<!-- Issue PAY-2473 --> O uso de [chaves do Composer incorretas](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26826) durante a instalação da extensão impede que o usuário [autentique](https://experienceleague.adobe.com/en/docs/commerce-operations/installation-guide/prerequisites/authentication-keys) com o `MAGEID` correto.

![Problema conhecido](../assets/bug.svg)<!-- Issue PAY-2474 --> [!DNL Payment Services] relatórios [podem não sincronizar imediatamente](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26837).

![Problema conhecido](../assets/bug.svg)<!-- Issue PAY-2475 --> Sua [conta de sandbox do PayPal](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26836) para [!DNL Payment Services] não pode ser verificada se você criar essa conta durante a integração.
