...
Para otimizar a conversão e a receita, implemente **regras de pesquisa** efetivas, uma **regra de listagem padrão** forte e **[regras de categoria](add.md#category-rules)** (beta). Ajuste classificações usando dados de vendas, estoque, promoções e [classificação inteligente](add.md#intelligent-ranking).
...
- Use as [regras de categoria](add.md#category-rules) em **páginas de categoria** de tráfego alto ou margem alta, em que a ordem com curadoria é importante tanto quanto a pesquisa, por exemplo, coleções sazonais ou departamentos em destaque.
