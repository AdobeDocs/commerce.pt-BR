---
user-guide-title: '[!DNL App Management]'
user-guide-description: Associe, configure e gerencie aplicativos do App Builder com sua instância do Adobe Commerce.
feature: App Builder, Extensibility, Integration
source-git-commit: ab635fecb7b82294bd4a4fd045ed71931e9d265d
workflow-type: tm+mt
source-wordcount: '27'
ht-degree: 7%

---


# [!DNL App Management] {#app-management}

- [Introdução](overview.md)
- [Instalar](install.md)
- Gerencie seu aplicativo {#manage-app}
   - [Gerencie seu aplicativo](manage-app.md)
   - [Solução de problemas](troubleshooting.md)
- [Retornar à Página Inicial dos Serviços Commerce](https://experienceleague.adobe.com/pt-br/docs/commerce/user-guides/home)
